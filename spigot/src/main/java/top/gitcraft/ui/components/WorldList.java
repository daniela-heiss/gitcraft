...
public class WorldList {
    ...
    public static String worldListSelectSubset(LISTTYPE type, List<String> worldNamesAll, int page, String mergeType, String fromOrIntoSelect, String fromWorld, String intoWorld) {
        int ifRest = worldNamesAll.size() % 9 != 0 ? 1 : 0;
        int maxPage = (worldNamesAll.size() / 9) + ifRest;
        if (worldNamesAll.isEmpty()){ // theoretically unreachable, just here for
            page = 1;
        ...
        jsonBuilder.text(symbol)
                .text("[").bold()
                .text(type.name().toUpperCase()).bold().color(type.getColor()).click(CLICKACTION.run_command, "/gcmergemenu" + " : " + mergeType + " " + fromOrIntoSelect + " " + Bukkit.getWorld(fromWorld).getName() + " " + Bukkit.getWorld(intoWorld).getName() + " " + worldNames.get(0)).hover(HOVERACTION.show_text, "Click to " + type.name().toLowerCase() + " " + worldNames.get(0))
                .text("] ").bold()
                .text(worldNames.get(0)).bold()
                .spacing(1);
        ...
        for (String worldName : worldNames) {
            jsonBuilder.text("\\u2560")
                    .text("[").bold()
                    .text(type.name().toUpperCase()).bold().color(type.getColor()).click(CLICKACTION.run_command, "/gcmergemenu" + " : " + mergeType + " " + fromOrIntoSelect + " " + Bukkit.getWorld(fromWorld).getName() + " " + Bukkit.getWorld(intoWorld).getName() + " " + worldName).hover(HOVERACTION.show_text, "Click to " + type.name().toLowerCase() + " " + worldName)
                    .text("] ").bold()
                    .text(worldName).bold()
                    .spacing(1);
        }
        ...
        if(!(lastWorld == null)) {
            jsonBuilder.text("\\u255a")
                    .text("[").bold()
                    .text(type.name().toUpperCase()).bold().color(type.getColor()).click(CLICKACTION.run_command, "/gcmergemenu" + " : " + mergeType + " " + fromOrIntoSelect + " " + Bukkit.getWorld(fromWorld).getName() + " " + Bukkit.getWorld(intoWorld).getName() + " " + lastWorld).hover(HOVERACTION.show_text, "Click to " + type.name().toLowerCase() + " " + lastWorld)
                    .text("] ").bold()
                    .text(lastWorld).bold();
        }
        jsonBuilder.spacing(9-(worldNames.size()));
                                                                                                                                                                                                            // dispatchTellRawCommand(player, worldLisSelectAll(LISTTYPE.SELECT, 1, args[1], args[2], args[3], args[4]));
        String leftArrow = page == 1 ? new JsonBuilder().text("⏪ ◁ ").bold().color(JSONCOLOR.GRAY).build() : new JsonBuilder().text("⏪ ").bold().hover(HOVERACTION.show_text, "First page").click(CLICKACTION.run_command, "/gcmergemenu"+ " :: " + 1 + " " + mergeType + " " + fromOrIntoSelect + " " + fromWorld + " " + intoWorld).text("◀ ").bold().hover(HOVERACTION.show_text, "Previous page").click(CLICKACTION.run_command, "/gcmergemenu"+ " :: " + (page-1) + " " + mergeType + " " + fromOrIntoSelect + " " + fromWorld + " " + intoWorld).build();
        String rightArrow = page == maxPage ? new JsonBuilder().text(" ▷ ⏩").bold().color(JSONCOLOR.GRAY).build() : new JsonBuilder().text(" ▶ ").bold().hover(HOVERACTION.show_text, "Next page").click(CLICKACTION.run_command, "/gcmergemenu"+ " :: " + (page+1) + " " + mergeType + " " + fromOrIntoSelect + " " + fromWorld + " " + intoWorld).text("⏩").bold().hover(HOVERACTION.show_text, "Last page").click(CLICKACTION.run_command, "/gcmergemenu"+ " :: " + maxPage + " " + mergeType + " " + fromOrIntoSelect + " " + fromWorld + " " + intoWorld).build();
    ...
    public static String worldLisSelectAll(LISTTYPE type, int page, String mergeType, String fromOrIntoSelect, String fromWorld, String intoWorld) {
        List<String> worldNames = new ArrayList<>();
        for (World world : Bukkit.getWorlds()) {
            worldNames.add(world.getName());
        }
        return worldListSelectSubset(type, worldNames, page, mergeType, fromOrIntoSelect, fromWorld, intoWorld);
    }
}
